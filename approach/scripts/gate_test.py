...
class Gate:
    ...
    def finish_job(self):
        ...
        if self.status == 3 and self.trial ==2:
            ...
            if self.is_it_right_side(0.5):
                ...
                print("Better luck next time start from beginning :(")
                self.markers = {}
                self.trial = 0
                self.initalized_finish = False
    ...
    def is_it_right_side(self,threshold):
        ...
        marker1_yaw = euler_from_quaternion((marker1_ori.x,marker1_ori.y,marker1_ori.z,marker1_ori.w))[1] # change pitch and yaw [1] == pitch
        marker2_yaw = euler_from_quaternion((marker2_ori.x,marker2_ori.y,marker2_ori.z,marker2_ori.w))[1] # change pitch and yaw [1] == pitch
    ...
    def marker(self,data):
        ...
        if data.id not in self.count_marker.keys():
            ...
            send_goal(0,0,m.atan2(self.correct_pose.position.y,self.correct_pose.position.x),"base_link")
            self.stop()
        ...
        for key,value in self.count_marker.items():
            ...
            if value > 15:
                self.correct_pose.position.x += 0.5 # since camera is 50 cm in front of base link
                ...
                if(self.x != None):
                    self.correct_pose.position.x = x_rel + self.x
                    self.correct_pose.position.y = self.y +  y_rel
